...
select_freq <- function(df, tertile, qus, value = TRUE, var = "nei") {
  prova <- df %>%
    select(baby:var) %>%
  ...
  for (i in seq_along(prova$baby)) {
    ...
    prova[[var]][[i]] <- prova[[var]][[i]][prova[[tertile]][[i]]]
...
freq_pp_qus <- freq_qu_fun(mot_uni_on_subtlex_us$phon, chi_uni_on_subtlex_us$phon)
