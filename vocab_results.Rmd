...
freq_len_pp_fun <- function(df, pp_qu, le, general, freq_df, qus) {
  ...
  if (pp_qu == 1) {
    df_output %<>%
      filter(ph_pr < pp_qu_subtlex_us[2])  # PP qu
  } else if (pp_qu == 2) {
    df_output %<>%
      filter(ph_pr >= pp_qu_subtlex_us[2] & ph_pr < pp_qu_subtlex_us[3]) # PP qu
  } else {
    df_output %<>%
      filter(ph_pr >= pp_qu_subtlex_us[3]) # PP qu
  }
    df_output %<>%
...
freq_len_nd_fun <- function(df, nd_qu, le, general, freq_df, qus) {
  ...
  if (nd_qu == 1) {
    df_output %<>%
      filter(nei < nei_qu[2])  # ND qu
  } else if (nd_qu == 2) {
    df_output %<>%
      filter(nei >= nei_qu[2] & nei < nei_qu[3]) # ND qu
  } else {
    df_output %<>%
      filter(nei >= nei_qu[3]) # ND qu
  }
  df_output %<>%
    mutate(len = str_split(phon, "_") %>%
             sapply(length)) %>%
...
### Phonotactic probability at each general frequency tertile
...
### Phonotactic probability at each maternal frequency tertile
...
### Neighbourhood density at each phonemic word length
...
### Neighbourhood Density at each general frequency tertile
...
### Neighbourhood Density at each maternal frequency tertile
...
### General frequency at each phonemic length (for each different PP tertile)
...
### General frequency at each phonemic length (for each different ND tertile)
...
### Maternal frequency at each phonemic length (for each different ND tertile)
